...
export function normalizeProject(value: unknown, now = new Date()): unknown {
  ...
  const components = Array.isArray(value.components) ? value.components.map((item) => isRecord(item) ? { ...item, description: typeof item.description === "string" ? item.description : "", notes: typeof item.notes === "string" ? item.notes : "" } : item) : [];
