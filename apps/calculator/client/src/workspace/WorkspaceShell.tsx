...
export function WorkspaceShell() {
  const workspace = useWorkspace(); const { project, localState, saveState } = workspace; const [location, setLocation] = useLocation(); const fileRef = useRef<HTMLInputElement>(null); const [headerErrors, setHeaderErrors] = useState<string[]>([]);
  ...
  if (localState === "invalid" && !project) return <>{fileInput}<InvalidLocalProjectState onOpen={open} /></>;
  if (!project) return <>{fileInput}<NoProjectState onOpen={open} /></>;
